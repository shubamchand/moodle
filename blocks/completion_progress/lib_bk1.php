...
function block_completion_progress_student_submissions($courseid, $userid) {
    ...
    $queries = array (
        ...
        . "AND m.name = 'quiz'"
        . "AND m.id = c.module "
        . "AND c.instance = q.id AND qa.state='finished'  "
    );
    ...
    foreach ($activities as $activity) {
        ...
        if ($completions[$activity['id']] == COMPLETION_INCOMPLETE && in_array($activity['id'], $submissions)) {
            if ($cm = get_coursemodule_from_id('quiz', $activity['id'])) {
                $SQL = "SELECT * FROM {quiz_attempts} where quiz=:quizid and userid=:userid and state=:state order by attempt desc limit 0,1";
                $attempt = $DB->get_record_sql($SQL, array('quizid' => $cm->instance, 'userid' => $userid, 'state' => 'finished'));
              
                if ($attempt and $attempt->sumgrades != NULL) {
