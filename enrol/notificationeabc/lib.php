...
class enrol_notificationeabc_plugin extends enrol_plugin
{
    ...
    public function process_mensaje($mensaje, stdClass $user, stdClass $course) {
        ...
        $m = str_replace('{SITEURL}', $CFG->wwwroot, $m);
        $m = str_replace('{SITENAME}', $SITE->fullname, $m);

        if ( function_exists('theme_ausinet_get_setting') ) {
            $m = str_replace('{SITELOGO}', theme_ausinet_get_setting('logo', 'file') , $m);
        }
