...
class mod_recommend_request_manager {
    ...
    public function add_requests($data) {
        ...
        for ($i = 1; $i <= $canadd; $i++) {
            if (!empty($data->{'email'.$i})) {
                $email = $data->{'email'.$i};
                $name = $data->{'name'.$i};
                $phone_number = $data->{'phone_number'.$i}; // added by nirmal recommend
                ...
                $this->add_request($email, $name,$phone_number); // update by nirmal recommend
            }
        }
    }
     ...
     * @return string
     */
    public static function generate_secret($userid, $email, $name,$phone_number) { // updated by nirmal recommend
        global $DB;
        while (true) {
            $secret = md5('secret/'.rand(1, 10000).'/'.$userid.'/'.$email.'/'.$name.'/'.$phone_number); // updated by nirmal recommend
     ...
     * @return stdClass
     */
    protected function add_request($email, $name,$phone_number) { // update by nirmal recommend
        global $DB;
        $this->get_requests();
        $record = (object)array(
            'recommendid' => $this->object->id,
            ...
            'email' => $email,
            'name' => $name,
            'phone_number' => $phone_number, // added by nirmal recommend
            'status' => self::STATUS_PENDING,
            'timerequested' => time(),
            'timecompleted' => null,
            'secret' => $this->generate_secret($this->get_userid(), $email, $name,$phone_number) // update by nirmal recommend
        );
