    ...
    public function add_to_form($mform, $recommendation, $data, $mode) {
        ...
        if ($question->type === 'label') {
            ...
            if ($question->addinfo === 'name' && method_exists($recommendation, 'get_request_name')) {
             ...
             if ($question->addinfo === 'phone_number' && method_exists($recommendation, 'get_request_phone_number')) {
                $mform->setDefault($elementlabel, $recommendation->get_request_phone_number());
            }
            if ($question->addinfo === 'current_date' && method_exists($recommendation, 'get_default_current_date')) { // added by nirmal recommend
                $mform->setDefault($elementlabel, $recommendation->get_default_current_date());
            }
    ...
    public static function add_edit_elements(MoodleQuickForm $mform, $data) {
        ...
        if ($data->type === 'textfield') {
            $prefill = [
                ...
                'current_date' => get_string('current_date', 'mod_recommend'),
                'phone_number' => get_string('phone_number', 'mod_recommend'),
            ];
