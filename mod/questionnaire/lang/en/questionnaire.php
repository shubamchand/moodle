...
// changes added here with new vairable coursecode to display shortname field in the email template and formated the string as well
$string['submissionnotificationhtmluser'] = 'New <a href="{$a->submissionurl}">submission</a> to the questionnaire {$a->name}.<br /><strong>Student Name</strong> : <a href="{$a->profileurl}">{$a->username}</a><br /><strong>Course</strong> : {$a->coursename}<br /><strong>Course Code</strong> : {$a->coursecode}';
$string['submissionnotificationtextuser'] = 'New submission ({$a->submissionurl})  to the questionnaire "{$a->name}" questionnaire <br />Student Name : {$a->username} ({$a->profileurl}) <br /> Course : {$a->coursename}<br />Course Code : {$a->coursecode}.';
