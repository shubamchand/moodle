...
$url = new moodle_url('/mod/observation/fbsections.php', ['id' => $id]);
$PAGE->set_url($url);
...
if ($sectionid) {
    ...
    $feedbacksection = new mod_observation\feedback\section(['id' => $sectionid], $observation->questions);

} else if (!$DB->count_records('observation_fb_sections', ['surveyid' => $observation->sid])) {
    ...
    $feedbacksection = new mod_observation\feedback\section(['surveyid' => $observation->survey->id, 'sectionnum' => $section],
...
if ($action == 'removequestion') {
    ...
    if ($sectionid == $feedbacksection->id) {
        ...
        redirect(new moodle_url('/mod/observation/fbsections.php', ['id' => $cm->id]));
    }
}
...
$customdata->survey = $observation->survey;
$customdata->sectionselect = $DB->get_records_menu('observation_fb_sections', ['surveyid' => $observation->survey->id],
    'section', 'id,sectionlabel');

$feedbackform = new \mod_observation\feedback_section_form('fbsections.php', $customdata);
...
$currentinfo = file_prepare_draft_area($draftideditor, $context->id, 'mod_observation', 'sectionheading',
...
if ($feedbackform->is_cancelled()) {
    redirect(new moodle_url('/mod/observation/feedback.php', ['id' => $cm->id]));
}
...
if ($settings = $feedbackform->get_data()) {
    ...
    if (isset($settings->gotosection)) {
        if ($settings->navigatesections != $feedbacksection->id) {
            redirect(new moodle_url('/mod/observation/fbsections.php',
                ['id' => $cm->id, 'sectionid' => $settings->navigatesections]));
        }

    } else if (isset($settings->addnewsection)) {
        $newsection = mod_observation\feedback\section::new_section($observation->survey->id, $settings->newsectionlabel);
        redirect(new moodle_url('/mod/observation/fbsections.php', ['id' => $cm->id, 'sectionid' => $newsection->id]));
        ...
        if (isset($settings->addquestionselect) && ($settings->addquestionselect != 0)) {
            if ($observation->questions[$settings->addquestionselect]->supports_feedback_scores()) {
        ...
        $feedbacksection->sectionheading = file_save_draft_area_files((int)$settings->sectionheading['itemid'], $context->id,
            'mod_observation', 'sectionheading', $feedbacksection->id, ['subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0],
        ...
        for ($i = 0; $i <= $settings->feedbackboundarycount; $i++) {
            ...
            $feedbacktext = file_save_draft_area_files((int)$settings->feedbacktext[$i]['itemid'],
                $context->id, 'mod_observation', 'feedback', $fbid, ['subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0],
...
if ($action == 'confirmremovequestion') {
    ...
    $msgargs->qname = $observation->questions[$qid]->name;
    $msgargs->sname = $feedbacksection->sectionlabel;
    $msg = '<div class="warning centerpara"><p>' . get_string('confirmremovequestion', 'observation', $msgargs) . '</p></div>';
    $args = ['id' => $observation->cm->id, 'sectionid' => $sectionid];
    $urlno = new moodle_url('/mod/observation/fbsections.php', $args);
    $args['action'] = 'removequestion';
    $args['qid'] = $qid;
    $urlyes = new moodle_url('/mod/observation/fbsections.php', $args);
    ...
    $msg = '<div class="warning centerpara"><p>' .
        get_string('confirmdeletesection', 'observation', $feedbacksection->sectionlabel) . '</p></div>';
    $args = ['id' => $observation->cm->id, 'sectionid' => $sectionid];
    $urlno = new moodle_url('/mod/observation/fbsections.php', $args);
