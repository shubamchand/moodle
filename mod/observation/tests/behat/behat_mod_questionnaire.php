...
/**
 * Steps definitions related with the observation activity.
 *
...
;
/**
 * Observation-related steps definitions.
 *
 * @package    mod_observation
 * @category   test
 * @copyright  2016 Mike Churchward - Poet Open Source
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_observation extends behat_base {

    /**
     * Adds a question to the observation with the provided data.
     *
    ...
    /**
     * Adds a questions and responses to the observation with the provided name.
     *
     * @Given /^"([^"]*)" has questions and responses$/
     *
     * @param string $observationname The name of an existing observation.
     */
    public function has_questions_and_responses($observationname) {
        global $DB;

        if (!$observation = $DB->get_record('observation', array('name' => $observationname), 'id,sid')) {
            throw new ExpectationException('Invalid observation name specified.', $this->getSession());
        }

        if (!$DB->record_exists('observation_survey', array('id' => $observation->sid))) {
            throw new ExpectationException('Observation survey does not exist.', $this->getSession());
        }

        $this->add_question_data($observation->sid);
        $this->add_response_data($observation->id, $observation->sid);
    }

    /**
     * Adds a question data to the given survey id.
     *
     * @param int $sid The id field of an existing observation_survey record.
     * @return null
     */
    private function add_question_data($sid) {
        ...
        $this->add_data($questiondata, 'observation_question', 'questionmap');
        $this->add_data($choicedata, 'observation_quest_choice', 'choicemap', array('questionmap' => 'question_id'));
    }

    /**
     * Adds response data to the given observation and survey id.
     *
     * @param int $qid The id field of an existing observation record.
     * @param int $sid The id field of an existing observation_survey record.
     * @return null
     */
    private function add_response_data($qid, $sid) {
        ...
        $responses = array(
            array("id", "observationid", "submitted", "complete", "grade", "userid"),
        ...
        $this->add_data($responses, 'observation_response', 'responsemap');
        ...
        $this->add_data($responsebool, 'observation_response_bool', '',
        ...
        $this->add_data($responsedate, 'observation_response_date', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id'));
        ...
        $this->add_data($responseother, 'observation_response_other', '',
        ...
        $this->add_data($responserank, 'observation_response_rank', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
        ...
        $this->add_data($respmultiple, 'observation_resp_multiple', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
        ...
        $this->add_data($respsingle, 'observation_resp_single', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
    }
