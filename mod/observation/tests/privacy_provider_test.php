...
/**
 * Privacy test for the mod observation.
 *
...
class mod_observation_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    /**
     * Test that only users with a observation context are fetched.
     */
    public function test_get_users_in_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist($cmcontext, 'mod_observation');
        ...
        $userlist = new \core_privacy\local\request\userlist(context_system::instance(), 'mod_observation');
        provider::get_users_in_context($userlist);
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new core_privacy\local\request\approved_contextlist($user, 'mod_observation', [$cmcontext->id]);
        provider::export_user_data($approvedlist);
        $data = $writer->get_data([]);

        $this->assertContains($observation->name, strip_tags($data->name));
        $this->assertEquals($observation->intro, strip_tags($data->intro));
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('observation_response', []));

        // Delete everything for the context.
        provider::delete_data_for_all_users_in_context($cmcontext);
        $this->assertCount(0, $DB->get_records('observation_response', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user, 'observation_response', [$cmcontext->id]);
        provider::delete_data_for_user($approvedlist);

        $this->assertCount(0, $DB->get_records('observation_response', ['userid' => $user->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('observation_response', []));
    }
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($cmcontext, 'observation', [$user->id, $user3->id]);

        // Get all accounts. There should be three.
        $this->assertCount(3, $DB->get_records('observation_response', []));
