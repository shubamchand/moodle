...
/**
 * Contains class mod_observation\search\question
 *
 ...
 */
namespace mod_observation\search;
defined('MOODLE_INTERNAL') || die();
/**
 * Search area for mod_observation questions. Separated from the activity search so that admins can choose whether or not they
...
class question extends \core_search\base_mod {
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        // Join the survey record to ensure only observations with questions are returned.
        $sql = 'SELECT q.* ' .
            'FROM {observation} q ' .
     ...
     * questions associated with a observation.
     *
    ...
    public function get_document($record, $options = []) {
        ...
        try {
            $cm = $this->get_cm('observation', $record->id, $record->course);
        ...
        $recordset = $DB->get_recordset('observation_question', ['surveyid' => $record->sid, 'deleted' => 'n'],
            'id', 'id,content');
    ...
    public function check_access($id) {
        global $DB;
        try {
            $activity = $DB->get_record('observation', ['id' => $id], '*', MUST_EXIST);
            $cminfo = $this->get_cm('observation', $activity->id, $activity->course);
        ...
        // If the user has the ability to see questions beyond completing a observation, grant access.
        $context = \context_module::instance($cminfo->id);
        if (!(has_capability('mod/observation:readallresponses', $context) ||
              has_capability('mod/observation:readallresponseanytime', $context) ||
              has_capability('mod/observation:editquestions', $context))) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/observation/view.php', ['id' => $context->instanceid]);
    }
}