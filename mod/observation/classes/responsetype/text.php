...
class text extends responsetype {
    ...
    static public function response_table() {
        return 'observation_response_text';
    }
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        if ($anonymous) {
            $sql = 'SELECT t.id, t.response, r.submitted AS submitted, ' .
                    'r.observationid, r.id AS rid ' .
                    'FROM {'.static::response_table().'} t, ' .
                    '{observation_response} r ' .
    ...
    public function results_template() {
        return 'mod_observation/results_text';
    }
    ...
    public function get_results_tags($weights, $participants, $respondents, $showtotals = 1, $sort = '') {
        ...
        if (is_object(reset($weights))) {
            global $CFG, $SESSION, $observation, $DB;
            $viewsingleresponse = $observation->capabilities->viewsingleresponse;
            $nonanonymous = $observation->respondenttype != 'anonymous';
            if ($viewsingleresponse && $nonanonymous) {
                $currentgroupid = '';
                if (isset($SESSION->observation->currentgroupid)) {
                    $currentgroupid = $SESSION->observation->currentgroupid;
                }
                $url = $CFG->wwwroot.'/mod/observation/report.php?action=vresp&amp;sid='.$observation->survey->id.
            ...
            $nbresponses = 0;
            $sum = 0;
            $strtotal = get_string('totalofnumbers', 'observation');
            $straverage = get_string('average', 'observation');
    ...
    static public function response_select($rid) {
        ...
        $sql = 'SELECT q.id, q.content, a.response as aresponse '.
            'FROM {'.static::response_table().'} a, {observation_question} q '.
