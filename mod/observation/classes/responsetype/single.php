...
namespace mod_observation\responsetype;
...
class single extends responsetype {
    ...
    static public function response_table() {
        return 'observation_resp_single';
    }
     ...
     * @return array \mod_observation\responsetype\answer\answer An array of answer objects.
    ...
    static public function answers_from_webform($responsedata, $question) {
        ...
        if (isset($responsedata->{'q'.$question->id}) && isset($question->choices[$responsedata->{'q'.$question->id}])) {
            ...
            if ($question->choices[$responsedata->{'q'.$question->id}]->is_other_choice()) {
                $cname = 'q' . $question->id .
                    \mod_observation\question\choice\choice::id_other_choice_name($responsedata->{'q'.$question->id});
    ...
    static public function answers_from_appdata($responsedata, $question) {
        ...
        if (isset($responsedata->{$qname}[0]) && !empty($responsedata->{$qname}[0])) {
            ...
            if ($question->choices[$record->choiceid]->is_other_choice()) {
                $cname = \mod_observation\question\choice\choice::id_other_choice_name($record->choiceid);
                $record->value =
    ...
    /**
     * @param \mod_observation\responsetype\response\response|\stdClass $responsedata
     * @return bool|int
     * @throws \coding_exception
    ...
    public function insert_response($responsedata) {
        ...
        if (!$responsedata instanceof \mod_observation\responsetype\response\response) {
            $response = \mod_observation\responsetype\response\response::response_from_webform($responsedata, [$this->question]);
        } else {
            ...
            foreach ($response->answers[$this->question->id] as $answer) {
                if (isset($this->question->choices[$answer->choiceid])) {
                    if ($this->question->choices[$answer->choiceid]->is_other_choice()) {
                        ...
                        $DB->insert_record('observation_response_other', $record);
                    }
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        $sql = 'SELECT rt.id, qc.id as cid, qc.content ' .
               'FROM {observation_quest_choice} qc, ' .
        ...
        $sql = 'SELECT rt.id, rt.response, qc.content ' .
               'FROM {observation_response_other} rt, ' .
                    '{observation_quest_choice} qc ' .
    ...
    public function get_feedback_scores(array $rids) {
        ...
        $sql = 'SELECT response_id as rid, c.value AS score ' .
            ...
            'INNER JOIN {observation_quest_choice} c ON r.choice_id = c.id ' .
    ...
    public function results_template() {
        return 'mod_observation/results_choice';
    }
        ...
        if ($rows) {
            ...
            foreach ($rows as $idx => $row) {
                if (strpos($idx, 'other') === 0) {
                    ...
                    $content = \mod_observation\question\choice\choice::content_other_choice_display($ccontent);
                    $content .= ' ' . clean_text($answer);
                    $textidx = $content;
                    $counts[$textidx] = !empty($counts[$textidx]) ? ($counts[$textidx] + 1) : 1;
                } else {
                    $contents = observation_choice_values($row->content);
    ...
    static public function response_select($rid) {
        ...
        $sql = 'SELECT a.id, q.id as qid, q.content, c.content as ccontent, c.id as cid, o.response ' .
            ...
            'INNER JOIN {observation_question} q ON a.question_id = q.id ' .
            'INNER JOIN {observation_quest_choice} c ON a.choice_id = c.id ' .
            'LEFT JOIN {observation_response_other} o ON a.response_id = o.response_id AND c.id = o.choice_id ' .
            'WHERE a.response_id = ? ';
        ...
        foreach ($records as $row) {
            ...
            if (\mod_observation\question\choice\choice::content_is_other_choice($row->ccontent)) {
                $newrow['responses'][\mod_observation\question\choice\choice::id_other_choice_name($row->cid)] = $row->response;
            }
    ...
    static public function response_answers_by_question($rid) {
        ...
        $sql = 'SELECT r.id as id, r.response_id as responseid, r.question_id as questionid, r.choice_id as choiceid, ' .
            ...
            'LEFT JOIN {observation_response_other} o ON r.response_id = o.response_id AND r.question_id = o.question_id AND ' .
     ...
     * @author Guy Thomas
     * @param int|array $observationids One id, or an array of ids.
     * @param bool|int $responseid
     * @param bool|int $userid
     * @return array
     */
    public function get_bulk_sql($observationids, $responseid = false, $userid = false, $groupid = false, $showincompletes = 0) {
        global $DB;
        ...
        if (is_array($observationids)) {
            list($qsql, $params) = $DB->get_in_or_equal($observationids);
        } else {
            $qsql = ' = ? ';
        ...
        $sql .= "
            AND qr.observationid $qsql $showcompleteonly
      LEFT JOIN {observation_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrs.choice_id
