...
use mod_observation\db\bulk_sql_config;
...
class rank extends responsetype {
    ...
    static public function response_table() {
        return 'observation_response_rank';
    }
    ...
    static public function answers_from_webform($responsedata, $question) {
        ...
        foreach ($question->choices as $cid => $choice) {
            ...
            if ($other == get_string('notapplicable', 'observation')) {
                $rank = -1;
            } else {
                $rank = intval($other);
            }
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        if ($rows = $DB->get_records_select('observation_quest_choice', $select)) {
            foreach ($rows as $row) {
        ...
        if (!$isrestricted) {
            if (!empty ($rankvalue)) {
                ...
                FROM {observation_quest_choice} c, {".static::response_table()."} r
            ...
            $sql = "SELECT c.id, c.content, a.average, a.num
                    FROM {observation_quest_choice} c
                    INNER JOIN
                         (SELECT c2.id, AVG(a2.rankvalue) AS average, COUNT(a2.response_id) AS num
                          FROM {observation_quest_choice} c2, {".static::response_table()."} a2
    ...
    public function display_results($rids=false, $sort='', $anonymous=false) {
        ...
        if ($rows = $this->get_results($rids, $sort, $anonymous)) {
                ...
                if (array_key_exists($ccontent, $rows)) {
                    ...
                    if (isset($rows[$ccontent]->averagevalue)) {
                        ...
                        if ($stravgvalue == '' && !$osgood) {
                            $stravgvalue = ' ('.get_string('andaveragevalues', 'observation').')';
                        }
                    } else {
                        $avgvalue = null;
                    }
            ...
            $output .= \mod_observation\responsetype\display_support::mkresavg($this->counts, count($rids),
                $this->question, $prtotal, $sort, $stravgvalue);

            $output .= \mod_observation\responsetype\display_support::mkrescount($this->counts, $rids, $rows, $this->question,
                $sort);
        } else {
            $output .= '<p class="generaltable">&nbsp;'.get_string('noresponsedata', 'observation').'</p>';
        }
        return $output;
    }
    ...
    static public function response_select($rid) {
        ...
        $sql = 'SELECT a.id as aid, q.id AS qid, q.precise AS precise, c.id AS cid, q.content, c.content as ccontent,
            ...
            'FROM {'.static::response_table().'} a, {observation_question} q, {observation_quest_choice} c '.
        ...
        foreach ($records as $row) {
            ...
            $osgood = false;
            if (\mod_observation\question\rate::type_is_osgood_rate_scale($row->precise)) {
                $osgood = true;
            }
                ...
                if ($key != 'content') { // No need to keep question text - ony keep choice text and rank.
                    if ($key == 'ccontent') {
                        if ($osgood) {
                            ...
                            $contents = observation_choice_values($val);
                            if ($contents->modname) {
