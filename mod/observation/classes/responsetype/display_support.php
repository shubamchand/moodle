...
class display_support {
    ...
    public static function mkresavg($counts, $total, $question, $showtotals, $sort, $stravgvalue='') {
        global $CFG;
        $stravgrank = get_string('averagerank', 'observation');
        $osgood = false;
        if ($question->precise == 3) { // Osgood's semantic differential.
            $osgood = true;
            $stravgrank = get_string('averageposition', 'observation');
        }
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'observation');
        }
        ...
        if (!empty($counts) && is_array($counts)) {
            ...
            $table->data[] = array('', get_string('noresponsedata', 'observation'));
        }
    ...
    public static function mkrescount($counts, $rids, $rows, $question, $sort) {
        ...
        $strtotal = '<strong>'.get_string('total', 'observation').'</strong>';
        $isna = $question->precise == 1;
        $isnahead = '';
        $osgood = false;
        if ($question->precise == 3) { // Osgood's semantic differential.
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'observation').'<br />(#)';
        }
        ...
        $headings = array('<span class="smalltext">'.get_string('responses', 'observation').'</span>');
        if ($osgood) {
            $align = array('right');
        } else {
            $align = array('left');
        ...
        if ($isrestricted) {
            array_push($headings, get_string('notapplicable', 'observation'));
            array_push($align, 'center');
        }
        array_push($align, 'center');
        if ($na) {
