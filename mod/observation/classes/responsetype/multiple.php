...
class multiple extends single {
    ...
    static public function response_table() {
        return 'observation_resp_multiple';
    }
            ...
            foreach ($responsedata->{'q' . $question->id} as $cid => $cvalue) {
                ...
                if (isset($question->choices[$cid])) {
                    ...
                    if ($question->choices[$cid]->is_other_choice()) {
                        $cname = \mod_observation\question\choice\choice::id_other_choice_name($cid);
            ...
            foreach ($responsedata->{$qname} as $choiceid => $choicevalue) {
                if ($choicevalue) {
                    ...
                    if (isset($question->choices[$choiceid]) && $question->choices[$choiceid]->is_other_choice()) {
                        $cname = \mod_observation\question\choice\choice::id_other_choice_name($choiceid);
                        $record->value =
        ...
        if (!empty($records)) {
            ...
            foreach ($records as $row) {
                ...
                if (\mod_observation\question\choice\choice::content_is_other_choice($row->ccontent)) {
                    $newrow['responses'][\mod_observation\question\choice\choice::id_other_choice_name($row->cid)] =
                        $row->response;
                }
            }
            $values[$qid] = $newrow;
    ...
    public function get_bulk_sql($observationids, $responseid = false, $userid = false, $groupid = false, $showincompletes = 0) {
      ...
      LEFT JOIN {observation_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrm.choice_id
