...
class essay extends text {
    ...
    protected function question_survey_display($response, $descendantsdata, $blankobservation=false) {
        ...
        if ($canusehtmleditor) {
            ...
            $editor->use_editor($name, observation_get_editor_options($this->context));
    ...
    protected function form_length(\MoodleQuickForm $mform, $helptext = '') {
        $responseformats = array(
                        "0" => get_string('formateditor', 'observation'),
                        "1" => get_string('formatplain', 'observation'));
        $mform->addElement('select', 'precise', get_string('responseformat', 'observation'), $responseformats);
    ...
    protected function form_precise(\MoodleQuickForm $mform, $helptext = '') {
        ...
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'observation', $lines);
        }
        $mform->addElement('select', 'length', get_string('responsefieldlines', 'observation'), $choices);
