...
use mod_observation\edit_question_form;
use \observation;
...
/**
 * This file contains the parent class for observation question types.
 *
...
abstract class question {
    ...
    /** @var int $position Position in the observation */
    public $position    = 0;
    ...
    public function __construct($id = 0, $question = null, $context = null, $params = []) {
        ...
        if ($qtypes === null) {
            $qtypes = $DB->get_records('observation_question_type', [], 'typeid',
                                       'typeid, type, has_choices, response_table');
        }

        if ($id) {
            $question = $DB->get_record('observation_question', ['id' => $id]);
        }
    ...
    static public function question_builder($qtype, $qdata = null, $context = null) {
        $qclassname = '\\mod_observation\\question\\'.self::qtypename($qtype);
        $qid = 0;
    ...
    private function get_choices() {
        ...
        if ($choices = $DB->get_records('observation_quest_choice', ['question_id' => $this->id], 'id ASC')) {
    ...
    private function get_dependencies() {
        ...
        $this->dependencies = [];
        $dependencies = $DB->get_records('observation_dependency',
    ...
    public function insert_response($responsedata) {
        if (isset($this->responsetype) && is_object($this->responsetype) &&
            is_subclass_of($this->responsetype, '\\mod_observation\\responsetype\\responsetype')) {
     ...
     *
     * @return object The response object based off of observation_response_base.
     *
     */
    ...
    public function update($questionrecord = null, $updatechoices = true) {
        ...
        $DB->update_record('observation_question', $questionrecord);
    ...
    public function add($questionrecord, array $choicerecords = null, $calcposition = true) {
        ...
        $this->qid = $DB->insert_record('observation_question', $questionrecord);
    ...
    public function update_choice($choicerecord) {
        global $DB;
        return $DB->update_record('observation_quest_choice', $choicerecord);
    }
    ...
    public function add_choice($choicerecord) {
        global $DB;
        $retvalue = true;
        if ($cid = $DB->insert_record('observation_quest_choice', $choicerecord)) {
    ...
    public function delete_choice($choice) {
        ...
        if (\mod_observation\question\choice\choice::delete_from_db_by_id($cid)) {
    ...
    public function insert_extradata($extradata) {
        global $DB;
        return $DB->set_field('observation_question', 'extradata', $extradata, ['id' => $this->id]);
    }
    ...
    public function update_dependency($dependencyrecord) {
        global $DB;
        return $DB->update_record('observation_dependency', $dependencyrecord);
    }
    ...
    public function add_dependency($dependencyrecord) {
        ...
        $retvalue = true;
        if ($did = $DB->insert_record('observation_dependency', $dependencyrecord)) {
    ...
    public function delete_dependency($dependency) {
        ...
        if ($DB->delete_records('observation_dependency', ['id' => $did])) {
    ...
    public function set_required($required) {
        ...
        $this->required = $rval;
        return $DB->set_field('observation_question', 'required', $rval, ['id' => $qid]);
    }
     ...
     *
     */
    abstract protected function question_survey_display($formdata, $descendantsdata, $blankobservation);
     ...
     * @return \stdClass
     */
    public function question_output($response, $dependants=[], $qnum='', $blankobservation) {
        $pagetags = $this->questionstart_survey_display($qnum, $response);
        $pagetags->qformelement = $this->question_survey_display($response, $dependants, $blankobservation);
        return $pagetags;
    }
    ...
    public function questionstart_survey_display($qnum, $response=null) {
        global $OUTPUT, $SESSION, $observation, $PAGE;
        ...
        $nonumbering = false;
        if (!$observation->questions_autonumbered()) {
            $qnum = '';
            $nonumbering = true;
        }

        // For now, check what the response type is until we've got it all refactored.
        if ($response instanceof \mod_observation\responsetype\response\response) {
            $skippedquestion = !isset($response->answers[$this->id]);
        } else {
            $skippedquestion = !empty($response) && !array_key_exists('q'.$this->id, $response);
        }

        // If we are on report page and this observation has dependquestions and this question was skipped.
        if (($pagetype == 'mod-observation-myreport' || $pagetype == 'mod-observation-report') &&
        ...
        // In report mode, If observation is set to no numbering,
        // also hide answers to questions that have not been answered.
        $displayclass = 'qn-container';
        if ($pagetype == 'mod-observation-preview' || ($nonumbering &&
            ...
            $observation->load_parents($this);
            // Want this to come from the renderer, meaning we need $observation.
            $pagetags->dependencylist = $observation->renderer->get_dependency_html($this->id, $this->dependencies);
        }
        ...
        if ($this->type_id != QUESSECTIONTEXT) {
            ...
            if ($this->required()) {
                ...
                $required .= get_string('required', 'observation');
                $required .= html_writer::end_tag('div');
                $required .= html_writer::empty_tag('img', ['class' => 'req', 'title' => get_string('required', 'observation'),
                    'alt' => get_string('required', 'observation'), 'src' => $OUTPUT->image_url('req')]);
            }
        ...
        $content = format_text(file_rewrite_pluginfile_urls($this->content, 'pluginfile.php',
            $this->context->id, 'mod_observation', 'question', $this->id), FORMAT_HTML, $options);
     ...
     * @param observation $observation The observation being edited.
     * @return bool
     */
    public function edit_form(edit_question_form $form, observation $observation) {
        $mform =& $form->_form;
        ...
        if ($observation->navigate > 0) {
            $this->form_dependencies($form, $observation->questions);
        }
        ...
        if (isset($this->qid)) {
            $buttonarray[] = &$mform->createElement('submit', 'makecopy', get_string('saveasnew', 'observation'));
        }
    ...
    protected function form_header(\MoodleQuickForm $mform, $helpname = '') {
        ...
        if (isset($this->qid) && !empty($this->qid)) {
            $header = get_string('editquestion', 'observation', observation_get_type($this->type_id));
        } else {
            $header = get_string('addnewquestion', 'observation', observation_get_type($this->type_id));
        }
        if (empty($helpname)) {
        ...
        $mform->addHelpButton('questionhdredit', $helpname, 'observation');
    }
    ...
    protected function form_name(\MoodleQuickForm $mform) {
        $mform->addElement('text', 'name', get_string('optionalname', 'observation'),
                        ['size' => '30', 'maxlength' => '30']);
        $mform->setType('name', PARAM_TEXT);
        $mform->addHelpButton('name', 'optionalname', 'observation');
        return $mform;
    }
    ...
    protected function form_required(\MoodleQuickForm $mform) {
        ...
        $mform->addGroup($reqgroup, 'reqgroup', get_string('required', 'observation'), ' ', false);
        $mform->addHelpButton('reqgroup', 'required', 'observation');
        return $mform;
    }
    ...
    protected function form_dependencies($form, $questions) {
        ...
        if (count($dependencies) > 1) {
            $mform->addElement('header', 'dependencies_hdr', get_string('dependencies', 'observation'));
             ...
             * bottom up, starting at the lowest child question is a pain for large observations.
            ...
            $mform->addElement('static', 'mandatory', '',
                '<div class="dimmed_text">' . get_string('mandatory', 'observation') . '</div>');
            $selectand = $mform->createElement('select', 'dependlogic_and', get_string('condition', 'observation'),
                [get_string('answernotgiven', 'observation'), get_string('answergiven', 'observation')]);
            $selectand->setSelected('1');
            $groupitemsand = [];
            $groupitemsand[] =& $mform->createElement('selectgroups', 'dependquestions_and',
                get_string('parent', 'observation'), $dependencies);
            $groupitemsand[] =& $selectand;
            $groupand = $mform->createElement('group', 'selectdependencies_and', get_string('dependquestion', 'observation'),
                $groupitemsand, ' ', false);
            ...
            $mform->addElement('static', 'optional', '',
                '<div class="dimmed_text">' . get_string('optional', 'observation') . '</div>');
            $selector = $mform->createElement('select', 'dependlogic_or', get_string('condition', 'observation'),
                [get_string('answernotgiven', 'observation'), get_string('answergiven', 'observation')]);
            ...
            $groupor = $mform->createElement('group', 'selectdependencies_or', get_string('dependquestion', 'observation'),
                $groupitemsor, ' ', false);
    ...
    protected function form_question_text(\MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('editor', 'content', get_string('text', 'observation'), null, $editoroptions);
    ...
    protected function form_choices(\MoodleQuickForm $mform) {
        if ($this->has_choices()) {
            ...
            $mform->addElement('textarea', 'allchoices', get_string('possibleanswers', 'observation'), $options);
            $mform->setType('allchoices', PARAM_RAW);
            $mform->addRule('allchoices', null, 'required', null, 'client');
            $mform->addHelpButton('allchoices', $helpname, 'observation');
    ...
    static public function form_length_text(\MoodleQuickForm $mform, $helpname = '', $value = 0) {
        $mform->addElement('text', 'length', get_string($helpname, 'observation'), ['size' => '1'], $value);
        $mform->setType('length', PARAM_INT);
        if (!empty($helpname)) {
            $mform->addHelpButton('length', $helpname, 'observation');
        }
        return $mform;
    }
    ...
    static public function form_precise_text(\MoodleQuickForm $mform, $helpname = '', $value = 0) {
        $mform->addElement('text', 'precise', get_string($helpname, 'observation'), ['size' => '1']);
        $mform->setType('precise', PARAM_INT);
        if (!empty($helpname)) {
            $mform->addHelpButton('precise', $helpname, 'observation');
        }
        return $mform;
    }
     ...
     */
    public function form_update($formdata, $observation) {
        global $DB;
        ...
        if (!empty($formdata->qid)) {
            ...
            $formdata->content = file_save_draft_area_files($formdata->itemid, $observation->context->id, 'mod_observation',
            ...
            if ($observation->has_dependencies()) {
            ...
            $formdata->format  = $formdata->content['format'];
            $formdata->content = $formdata->content['text'];
            $content           = file_save_draft_area_files($formdata->itemid, $observation->context->id, 'mod_observation',
                'question', $this->qid, ['subdirs' => true], $formdata->content);
            $DB->set_field('observation_question', 'content', $content, ['id' => $this->qid]);
        }
    ...
    public function mobile_question_display($qnum, $autonum = false) {
        ...
        $mobiledata = (object)[
            ...
            'content' => format_text(file_rewrite_pluginfile_urls($this->content, 'pluginfile.php', $this->context->id,
                'mod_observation', 'question', $this->id), FORMAT_HTML, $options),
