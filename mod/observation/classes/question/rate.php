...
class rate extends question {
    ...
    protected function responseclass() {
        return '\\mod_observation\\responsetype\\rank';
    }
    ...
    public function question_template() {
        return 'mod_observation/question_rate';
    }
    ...
    public function response_template() {
        return 'mod_observation/response_rate';
    }
     ...
     * @param \mod_observation\responsetype\response\response $data
    ...
    protected function question_survey_display($response, $descendantsdata, $blankobservation=false) {
        ...
        if ($this->has_na_column()) {
            $na = get_string('notapplicable', 'observation');
        } else {
            $na = '';
        }
        ...
        if (($nbchoices > 1) && !$this->no_duplicate_choices() && !$blankobservation) {
        ...
        if ( ($num != $nbchoices) && ($num != 0) ) {
            $this->add_notification(get_string('checkallradiobuttons', 'observation', $nbchoices));
            $notcomplete = true;
        }
            ...
            if (isset($choice->content)) {
                ...
                if (($nbchoices > 1) && !$this->no_duplicate_choices()  && !$blankobservation) {
                    ...
                    if ($notcomplete && isset($response->answers[$this->id][$cid]) &&
                        ...
                        $title = get_string('pleasecomplete', 'observation');
                    }
                ...
                for ($j = 1; $j <= $this->length + $this->has_na_column(); $j++) {
                    ...
                    $col['colclass'] = $bg;
                    $col['colhiddentext'] = get_string('option', 'observation', $j);
    ...
    protected function response_survey_display($response) {
        ...
        if ($this->has_na_column()) {
            $cellobj = new \stdClass();
            $cellobj->bg = $bg;
            $cellobj->str = get_string('notapplicable', 'observation');
            $resptags->headers[] = $cellobj;
        }
    ...
    public function response_complete($responsedata) {
        if (!is_a($responsedata, 'mod_observation\responsetype\response\response')) {
            $response = \mod_observation\responsetype\response\response::response_from_webform($responsedata, [$this]);
        } else {
    ...
    protected function form_precise(\MoodleQuickForm $mform, $helptext = '') {
        $precoptions = array("0" => get_string('normal', 'observation'),
                             "1" => get_string('notapplicablecolumn', 'observation'),
                             "2" => get_string('noduplicates', 'observation'),
                             "3" => get_string('osgood', 'observation'));
        $mform->addElement('select', 'precise', get_string('kindofratescale', 'observation'), $precoptions);
        $mform->addHelpButton('precise', 'kindofratescale', 'observation');
    ...
    protected function form_extradata(\MoodleQuickForm $mform, $helpname = '') {
        ...
        $mform->addElement('textarea', 'allnameddegrees', get_string('allnameddegrees', 'observation'), $options);
        $mform->setDefault('allnameddegrees', $defaultvalue);
        $mform->setType('allnameddegrees', PARAM_RAW);
        $mform->addHelpButton('allnameddegrees', 'allnameddegrees', 'observation');
    ...
    protected function form_preprocess_data($formdata) {
        ...
        if (!empty($formdata->allnameddegrees)) {
            ...
            foreach ($nameddegreelines as $nameddegreeline) {
                ...
                if (($nameddegree = \mod_observation\question\choice\choice::content_is_named_degree_choice($nameddegreeline)) !==
                    false) {
    ...
    public function mobile_question_display($qnum, $autonum = false) {
        ...
        if ($this->has_na_column()) {
            $mobiledata->hasnacolumn = (object)['value' => -1, 'label' => get_string('notapplicable', 'observation')];
        }
        ...
        if (!empty($nameddegrees)) {
            if ($question->insert_nameddegrees($nameddegrees)) {
                ...
                foreach ($oldchoiceids as $choiceid) {
                    \mod_observation\question\choice\choice::delete_from_db_by_id($choiceid);
                }

                // First get all existing rank responses for this question.
                $responses = $DB->get_recordset('observation_response_rank', ['question_id' => $question->id]);
                ...
                foreach ($responses as $response) {
                    ...
                    if (isset($newvalues[$response->rankvalue])) {
                        $DB->set_field('observation_response_rank', 'rankvalue', $newvalues[$response->rankvalue],
                            ['id' => $response->id]);
                    }
                }
                $responses->close();
    ...
    public static function move_all_nameddegree_choices(int $surveyid = null) {
        ...
        $skip = false;
        if ($surveyid !== null) {
            $qids = $DB->get_records_menu('observation_question', ['surveyid' => $surveyid, 'type_id' => QUESRATE],
                '', 'id,surveyid');
            if (!empty($qids)) {
        ...
        if (!$skip) {
            $select = 'UPDATE {observation_response_rank} ' .
        ...
        $ratequests = $DB->get_recordset('observation_question', $args);
