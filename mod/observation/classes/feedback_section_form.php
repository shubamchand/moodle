...
class feedback_section_form extends \moodleform {
    ...
    public function definition() {
        ...
        $feedbacksections = $observation->survey->feedbacksections;
        $this->_feedbacks = $feedbacksection->sectionfeedback;
        $this->context = $observation->context;
        $mform    =& $this->_form;

        if ($survey->feedbacksections > 1) {
            $mform->addElement('header', 'feedbacksectionsheader', get_string('feedbacksections', 'observation'));
            $sselectgroup = [];
            $sselect = $mform->createElement('select', 'navigatesections', get_string('navigatetosection', 'observation'),
                $this->_customdata->sectionselect);
            $sselect->setSelected($feedbacksection->id);
            $sselectgroup[] = $sselect;
            $sselectgroup[] = $mform->createElement('submit', 'gotosection', get_string('navigatetosection', 'observation'));
            $mform->addGroup($sselectgroup, '');
            $addnewsectionarray = [];
            $addnewsectionarray[] = $mform->createElement('text', 'newsectionlabel',
                get_string('feedbacksectionlabel', 'observation'));
            $mform->setType('newsectionlabel', PARAM_TEXT);
            $addnewsectionarray[] = $mform->createElement('submit', 'addnewsection', get_string('addnewsection', 'observation'));
            $mform->addGroup($addnewsectionarray, '', get_string('feedbacksectionlabel', 'observation'));
        }

        if ($feedbacksections == 1) {
            $label = get_string('feedbackglobal', 'observation');
            $feedbackheading = get_string('feedbackglobalheading', 'observation');
            $feedbackmessages = get_string('feedbackglobalmessages', 'observation');
        } else {
            $label = $feedbacksection->sectionlabel;
            $feedbackheading = get_string('feedbacksectionheading', 'observation', $label);
            $feedbackmessages = get_string('feedbackmessages', 'observation', $label);
        }

        $mform->addElement('header', 'contenthdr', $feedbackheading);
        $mform->addElement('text', 'sectionlabel', get_string('feedbacksectionlabel', 'observation'),
            ['size' => '50', 'maxlength' => '50']);
        $mform->setType('sectionlabel', PARAM_TEXT);
        $mform->addRule('sectionlabel', null, 'required', null, 'client');
        $mform->addHelpButton('sectionlabel', 'feedbacksectionlabel', 'observation');

        $editoroptions = ['maxfiles' => EDITOR_UNLIMITED_FILES, 'trusttext' => true];
        $mform->addElement('editor', 'sectionheading', get_string('feedbacksectionheadingtext', 'observation'),
            null, $editoroptions);
        $mform->setType('sectionheading', PARAM_RAW);
        $mform->setDefault('feedbacknotes', $observation->survey->feedbacknotes);
        $mform->addHelpButton('sectionheading', 'feedbackheading', 'observation');

        if ($observation->survey->feedbacksections > 0) {
            // Sections.
            if ($survey->feedbacksections > 1) {
                $mform->addElement('header', 'fbsection_' . $feedbacksection->id,
                    get_string('feedbacksectionquestions', 'observation', $label));
                ...
                if (!empty($feedbacksection->scorecalculation)) {
                    ...
                    foreach ($feedbacksection->scorecalculation as $qid => $score) {
                        ...
                        $mform->addGroup($questionactions, '', $observation->questions[$qid]->name);
                        $counter++;
                    }
                }
                if (!empty($qvalid)) {
                    ...
                    $qselect[] = $mform->createElement('select', 'addquestionselect',
                        get_string('addquestiontosection', 'observation'), $qvalid);
                    $qselect[] = $mform->createElement('submit', 'addquestion', get_string('addquestion', 'observation'));
                    $mform->addGroup($qselect, '', get_string('addquestiontosection', 'observation'));
                }
            }
        }
        ...
        $mform->addHelpButton('feedbackhdr', 'feedback', 'observation');

        $mform->addElement('static', 'scoreboundarystatic1', get_string('feedbackscoreboundary', 'observation'), '100%');

        $repeatarray = [];
        $repeatedoptions = [];

        $repeatarray[] = $mform->createElement('editor', 'feedbacktext', get_string('feedback', 'observation'), null,
            ['maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $observation->context]);
        $repeatarray[] = $mform->createElement(
            'text', 'feedbackboundaries', get_string('feedbackscoreboundary', 'observation'), ['size' => 10]);
        ...
        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1, $repeatedoptions, 'boundary_repeats',
            'boundary_add_fields', 2, get_string('feedbackaddmorefeedbacks', 'observation'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore(
            $mform->createElement('editor', "feedbacktext[$nextel]", get_string('feedback', 'observation'), null,
                ['maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $observation->context]),
            'boundary_add_fields');
        $mform->insertElementBefore(
            $mform->createElement('static', 'scoreboundarystatic2', get_string('feedbackscoreboundary', 'observation'), '0%'),
            'boundary_add_fields');
        ...
        $buttonarray[] = $mform->createElement('submit', 'confirmdeletesection', get_string('deletesection', 'observation'));
    ...
    public function validation($data, $files) {
        ...
        while (!empty($data['feedbackboundaries'][$i])) {
            ...
            if (is_numeric($boundary) && $boundary <= 0) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorboundaryoutofrange', 'observation', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrororder', 'observation', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i] ) &&
                    ...
                    $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorjunkinboundary', 'observation', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] = get_string('feedbackerrorjunkinfeedback', 'observation', $i + 1);
            }
        }
        return $errors;
    }
