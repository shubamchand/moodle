...
class observation {
    ...
    /**
     * @var \mod_observation\question\question[] $quesitons
     */
    public $questions = [];
     ...
     *
     */
    public function __construct($id = 0, $observation = null, &$course, &$cm, $addquestions = true) {
        global $DB;

        if ($id) {
            $observation = $DB->get_record('observation', array('id' => $id));
        }

        if (is_object($observation)) {
            $properties = get_object_vars($observation);
        ...
        $this->course = $course;
        $this->cm = $cm;
        // When we are creating a brand new observation, we will not yet have a context.
        ...
        // Load the capabilities for this user and observation, if not creating a new one.
        if (!empty($this->cm->id)) {
            $this->capabilities = observation_load_capabilities($this->cm->id);
        }
    ...
    public function add_survey($sid = 0, $survey = null) {
        ...
        if ($sid) {
            $this->survey = $DB->get_record('observation_survey', array('id' => $sid));
    ...
    public function add_questions($sid = false) {
        ...
        $params = [$sid, 'n'];
        if ($records = $DB->get_records_select('observation_question', $select, $params, 'position')) {
            $sec = 1;
            $isbreak = false;
            foreach ($records as $record) {

                $this->questions[$record->id] = \mod_observation\question\question::question_builder($record->type_id,
                    $record, $this->context);
    ...
    public function add_user_responses($userid = null) {
        ...
        // Empty observations cannot have responses.
        if (empty($this->id)) {
            return;
        }
        ...
        foreach ($responses as $response) {
            $this->responses[$response->id] = mod_observation\responsetype\response\response::create_from_data($response);
        }
    }
    ...
    public function add_response($responseid) {
        ...
        $response = $DB->get_record('observation_response', ['id' => $responseid]);
        $this->responses[$response->id] = mod_observation\responsetype\response\response::create_from_data($response);
    }
    ...
    public function add_response_from_formdata($formdata) {
        $this->responses[0] = mod_observation\responsetype\response\response::response_from_webform($formdata, $this->questions);
    }
     ...
     * @param $appdata
     * @param int $sec
     * @return bool|\mod_observation\responsetype\response\response
     */
    public function build_response_from_appdata($appdata, $sec=0) {
        ...
        return mod_observation\responsetype\response\response::response_from_appdata($this->id, 0, $appdata, $questions);
    }

    /**
     * Add the renderer to the observation object.
    ...
    /**
     * Add the templatable page to the observation object.
    ...
    public function view() {
        ...
        $PAGE->requires->js_init_call('M.mod_observation.init_attempt_form', null, false, observation_get_js_module());
        ...
        if ($message !== false) {
            ...
            // Handle the main observation completion page.
            $quser = $USER->id;

            $msg = $this->print_survey($USER->id, $quser);

            // If Observation was submitted with all required fields completed ($msg is empty),
            // then record the submittal.
            $viewform = data_submitted($CFG->wwwroot."/mod/observation/complete.php");

            // If observation response for students.
            $studentemail = '';
            if (isset($viewform->studentid) && !empty($viewform->studentid) ) {
                $quser = (int)$viewform->studentid;
                $studentemail = \core_user::get_user($quser)->email;
            }
            ...
            if ($viewform && confirm_sesskey() && isset($viewform->submit) && isset($viewform->submittype) &&
                ...
                $params = array(
                    ...
                    'relateduserid' => $quser,
                    'anonymous' => $anonymous,
                    'other' => array('observationid' => $this->id)
                );
                $event = \mod_observation\event\attempt_submitted::create($params);
                $event->trigger();

                $this->submission_notify($this->rid, $studentemail, $quser);
    ...
    /**
     * Update the grade for this observation and user.
     *
     * @param $userid
     */
    private function update_grades($userid) {
        if ($this->grade != 0) {
            ...
            $observation->grade = $this->grade;
            $observation->cmidnumber = $this->cm->idnumber;
            $observation->courseid = $this->course->id;
            observation_update_grades($observation, $userid);
        }
    }
    ...
    *
    */
    public function view_response($rid, $referer= '', $blankobservation = false, $resps = '', $compare = false,
        ...
        if ($referer != 'print') {
            ...
            if ($this->survey->feedbacknotes) {
                $text = file_rewrite_pluginfile_urls($this->survey->feedbacknotes, 'pluginfile.php',
                    $this->context->id, 'mod_observation', 'feedbacknotes', $this->survey->id);
    ...
    public function view_all_responses() {
        ...
        if (!empty($this->responses)) {
            ...
            $this->page->add_to_page('responses', $this->renderer->all_response_output(get_string('noresponses', 'observation')));
        }
    ...
    public function user_can_take($userid) {
        ...
        if (!$this->is_active() || !$this->user_is_eligible($userid)) {
            return false;
        } else if ($this->qtype == OBSERVATIONUNLIMITED) {
            return true;
        } else if ($userid > 0) {
    ...
    /**
     * Return any message if the user cannot complete this observation, explaining why.
     * @param int $userid
    ...
    public function user_access_messages($userid = 0, $asnotification = false) {
        ...
        if (!$this->is_active()) {
            ...
            $message = get_string($msg, 'observation');

        } else if ($this->survey->realm == 'template') {
            $message = get_string('templatenotviewable', 'observation');

        } else if (!$this->is_open()) {
            $message = get_string('notopen', 'observation', userdate($this->opendate));

        } else if ($this->is_closed()) {
            $message = get_string('closed', 'observation', userdate($this->closedate));

        } else if (!$this->user_is_eligible($userid)) {
            $message = get_string('noteligible', 'observation');

        } else if (!$this->user_can_take($userid)) {
            switch ($this->qtype) {
                case OBSERVATIONDAILY:
                    $msgstring = ' ' . get_string('today', 'observation');
                    break;
                case OBSERVATIONWEEKLY:
                    $msgstring = ' ' . get_string('thisweek', 'observation');
                    break;
                case OBSERVATIONMONTHLY:
                    $msgstring = ' ' . get_string('thismonth', 'observation');
                    break;
                default:
                    $msgstring = '';
                    break;
            }
            $message = get_string("alreadyfilled", "observation", $msgstring);
        }
    ...
    public function user_has_saved_response($userid) {
        ...
        return $DB->record_exists('observation_response',
            ['observationid' => $this->id, 'userid' => $userid, 'complete' => 'n']);
    }

    public function user_time_for_new_attempt($userid) {
        global $DB;

        $params = ['observationid' => $this->id, 'userid' => $userid, 'complete' => 'y'];
        if (!($attempts = $DB->get_records('observation_response', $params, 'submitted DESC'))) {
            return true;
        }
        ...
        switch ($this->qtype) {
            ...
            case OBSERVATIONUNLIMITED:
                $cantake = true;
                break;

            case OBSERVATIONONCE:
                $cantake = false;
                break;
    ...
    public function can_view_response($rid) {
        ...
        if (!empty($rid)) {
            $response = $DB->get_record('observation_response', array('id' => $rid));
            ...
            if ($response->observationid != $this->id) {
                return false;
            }
            ...
            // If resp_view is set to OBSERVATION_STUDENTVIEWRESPONSES_NEVER, then this will always be false.
            if ($this->capabilities->readallresponses &&
                ($this->resp_view == OBSERVATION_STUDENTVIEWRESPONSES_ALWAYS ||
                 ($this->resp_view == OBSERVATION_STUDENTVIEWRESPONSES_WHENCLOSED && $this->is_closed()) ||
                 ($this->resp_view == OBSERVATION_STUDENTVIEWRESPONSES_WHENANSWERED  && !$this->user_can_take($USER->id)))) {
                return true;
            }
    ...
    /**
     * True if the user can view the responses to this observation, and there are valid responses.
    ...
    /**
     * True if the user can view all of the responses to this observation any time, and there are valid responses.
     * @param null $usernumresp
     * @return bool
     * @throws coding_exception
     */
    ...
    public function can_view_all_responses_with_restrictions($usernumresp, $grouplogic = true, $respslogic = true) {
        ...
        return $grouplogic && $respslogic && $this->is_survey_owner() &&
            ($this->capabilities->readallresponses &&
                ($this->resp_view == OBSERVATION_STUDENTVIEWRESPONSES_ALWAYS ||
                    ($this->resp_view == OBSERVATION_STUDENTVIEWRESPONSES_WHENCLOSED && $this->is_closed()) ||
                    ($this->resp_view == OBSERVATION_STUDENTVIEWRESPONSES_WHENANSWERED && $usernumresp)));
    ...
    public function count_submissions($userid=false, $groupid=0) {
        ...
        // Since submission can be across observations in the case of public observations, need to check the realm.
        // Public observations can have responses to multiple observation instances.
        if ($this->survey_is_public_master()) {
            $sql = 'SELECT COUNT(r.id) ' .
                'FROM {observation_response} r ' .
                'INNER JOIN {observation} q ON r.observationid = q.id ' .
                'INNER JOIN {observation_survey} s ON q.sid = s.id ' .
                $groupsql .
            ...
            $sql = 'SELECT COUNT(r.id) ' .
                'FROM {observation_response} r ' .
                $groupsql .
                'WHERE r.observationid = :observationid AND r.complete = :status' . $groupcnd;
            $params['observationid'] = $this->id;
            $params['status'] = 'y';
        }
        if ($userid) {
    ...
    /**
     * Get the requested responses for this observation.
     *
    ...
    /**
     * Check if current observation has dependencies set and any question has dependencies.
     *
    ...
    public function next_page($secnum, $rid) {
        ...
        if ($this->has_dependencies()) {
            while (!$this->eligible_questions_on_page($secnum, $rid)) {
                $secnum++;
                // We have reached the end of observation on a page without any question left.
    ...
    public function print_survey($userid=false, $quser) {
        ...
        // If student saved a "resume" observation OR left a observation unfinished
        ...
        $msg = '';
        $action = $CFG->wwwroot.'/mod/observation/complete.php?id='.$this->cm->id;
        ...
        // Save each section 's $formdata somewhere in case user returns to that page when navigating the observation.
        if (!empty($formdata->next)) {
            ...
            if ($msg) {
                ...
                if ($nextsec === false) {
                    $SESSION->observation->end = true; // End of observation reached on a no questions page.
        ...
        if (!empty($formdata->prev)) {
            ...
            // unlock observation->end to allow navigate back to previous page.
            if (isset($SESSION->observation->end) && ($SESSION->observation->end == true)) {
                $SESSION->observation->end = false;
                $formdata->sec--;
            }
        ...
        if (isset($this->questions) && $numsections) { // Sanity check.
            ...
            if ($formdata->sec > 1) {
                $controlbuttons['prev'] = ['type' => 'submit', 'class' => 'btn btn-secondary',
                    'value' => '<< '.get_string('previouspage', 'observation')];
            }
            if ($this->resume) {
                $controlbuttons['resume'] = ['type' => 'submit', 'class' => 'btn btn-secondary',
                    'value' => get_string('save', 'observation')];
            }
            ...
            if ($formdata->sec == $numsections) {
                ...
                $controlbuttons['submit'] = ['type' => 'submit', 'class' => 'btn btn-primary',
                    'value' => get_string('submitsurvey', 'observation')];
            } else {
                $controlbuttons['next'] = ['type' => 'submit', 'class' => 'btn btn-secondary',
                    'value' => get_string('nextpage', 'observation').' >>'];
            }
            ...
            $this->page->add_to_page('controlbuttons',
                $this->renderer->complete_controlbuttons(get_string('noneinuse', 'observation')));
        }
    ...
    private function survey_render($section = 1, $message = '', &$formdata) {
        ...
        if ($section > $numsections) {
            ...
            $this->page->add_to_page('notifications',
                $this->renderer->notification(get_string('finished', 'observation'), \core\output\notification::NOTIFY_WARNING));
        ...
        if ($this->page->cansubmit()) {
            $this->page->add_to_page('questions', $this->renderer->render_from_template('observation/studentlist', $this->get_student_list() ) ) ;
        }
        foreach ($this->questionsbysec[$section] as $questionid) {
            if ($this->questions[$questionid]->type_id != QUESSECTIONTEXT) {
                $i++;
            }
            // Need observation id to get the observation object in sectiontext (Label) question class.
            $formdata->observation_id = $this->id;
    ...
    private function print_survey_start($message, $section, $numsections, $hasrequired, $rid='', $blankobservation=false) {
        ...
        if ($rid) {
            $courseid = $this->course->id;
            if ($resp = $DB->get_record('observation_response', array('id' => $rid)) ) {
                if ($this->respondenttype == 'fullname') {
                    $userid = $resp->trainer_id; // LMSACE Changes
                    // Display name of group(s) that student belongs to... if observation is set to Groups separate or visible.
                    ...
                    $event = \mod_observation\event\response_viewed::create($params);
                    $event->trigger();
                }
            }
        }
        $ruser = '';
        if ($resp && !$blankobservation) {
            if ($userid) {
            ...
            if ($this->respondenttype == 'anonymous') {
                $ruser = '- '.get_string('anonymous', 'observation').' -';
            } else {
                // JR DEV comment following line out if you do NOT want time submitted displayed in Anonymous surveys.
                if ($resp->submitted) {
                    $timesubmitted = '&nbsp;'.get_string('submitted', 'observation').'&nbsp;'.userdate($resp->submitted);
                }
            }
        }
        if ($ruser) {
            $respinfo = get_string('respondent', 'observation').': <strong>'.$ruser.'</strong>';
            if ($this->survey_is_public()) {
                // For a public observation, look for the course that used it.
                $coursename = '';
                $sql = 'SELECT q.id, q.course, c.fullname ' .
                       'FROM {observation_response} qr ' .
                       'INNER JOIN {observation} q ON qr.observationid = q.id ' .
        ...
        if ($this->capabilities->printblank && $blankobservation && $section == 1) {
            ...
            $url = '/mod/observation/print.php?qid='.$this->id.'&amp;rid=0&amp;'.'courseid='.$this->course->id.'&amp;sec=1';
        ...
        if ($section == 1) {
            ...
            if ($this->survey->info) {
                $infotext = file_rewrite_pluginfile_urls($this->survey->info, 'pluginfile.php',
                    $this->context->id, 'mod_observation', 'info', $this->survey->id);
    ...
    private function print_survey_end($section, $numsections) {
        ...
        if ($numsections > 1) {
            ...
            $this->page->add_to_page('pageinfo',
                $this->renderer->container(get_string('pageof', 'observation', $a).'&nbsp;&nbsp;', 'surveyPage'));
        }
    }

    // Blankobservation : if we are printing a blank observation.
    public function survey_print_render($message = '', $referer='', $courseid, $rid=0, $blankobservation=false) {
        global $DB, $CFG;
        ...
        if (!empty($rid)) {
            ...
            $this->view_response($rid, $referer, $blankobservation);
            return;
        }
        ...
        $action = $CFG->wwwroot.'/mod/observation/preview.php?id='.$this->cm->id;
        ...
        if (data_submitted()) {
            ...
            foreach ($this->questionsbysec as $section) {
                ...
                if ($errormessage) {
                    if ($numsections > 1) {
                        $pageerror = get_string('page', 'observation').' '.$s.' : ';
                    }
                    $this->page->add_to_page('notifications',
        ...
        if ($errors == 0) {
            $this->page->add_to_page('message',
                $this->renderer->notification(get_string('submitpreviewcorrect', 'observation'),
        ...
        foreach ($this->questionsbysec as $section) {
            $output = '';
            if ($numsections > 1) {
                $output .= $this->renderer->print_preview_pagenumber(get_string('page', 'observation').' '.$page);
                $page++;
            }
        ...
        // End of questions.
        if ($referer == 'preview' && !$blankobservation) {
            $url = $CFG->wwwroot.'/mod/observation/preview.php?id='.$this->cm->id;
            $this->page->add_to_page('formend',
                $this->renderer->print_preview_formend($url, get_string('submitpreview', 'observation'), get_string('reset')));
        }
        return;
    }
    ...
    public function survey_update($sdata) {
        ...
        if (empty($this->survey->id)) {
            ...
            $this->survey->id = $DB->insert_record('observation_survey', $record);
            $this->add_survey($this->survey->id);

            if (!$this->survey->id) {
                $errstr = get_string('errnewname', 'observation') .' [ :  ]'; // TODO: notused!
                return(false);
            }
        } else {
            ...
            $name = $DB->get_field('observation_survey', 'name', array('id' => $this->survey->id));

            // Trying to change survey name.
            if (trim($name) != trim(stripslashes($sdata->name))) {  // $sdata will already have slashes added to it.
                $count = $DB->count_records('observation_survey', array('name' => $sdata->name));
                if ($count != 0) {
                    $errstr = get_string('errnewname', 'observation');  // TODO: notused!
                    return(false);
                }
            ...
            $result = $DB->update_record('observation_survey', $surveyrecord);
            if (!$result) {
                $errstr = get_string('warning', 'observation').' [ :  ]';  // TODO: notused!
                return(false);
            }
        }
    ...
    public function survey_copy($owner) {
        ...
        $i = 0;
        $name = $survey->name;
        while ($DB->count_records('observation_survey', array('name' => $name)) > 0) {
            $name = $survey->name.(++$i);
        }
        if ($i) {
            $survey->name .= $i;
        ...
        // Create new survey.
        if (!($newsid = $DB->insert_record('observation_survey', $survey))) {
            return(false);
        }
        ...
        foreach ($this->questions as $question) {
            ...
            foreach ($question->choices as $key => $choice) {
                ...
                if (!$newcid = $DB->insert_record('observation_quest_choice', $choice)) {
                    return(false);
                }
        ...
        $dependquestions = $DB->get_records('observation_dependency', ['surveyid' => $this->survey->id], 'questionid');
        foreach ($dependquestions as $dquestion) {
            ...
            $DB->insert_record('observation_dependency', $record);
        }

        // Replicate any feedback data.
        // TODO: Need to handle image attachments (same for other copies above).
        $fbsections = $DB->get_records('observation_fb_sections', ['surveyid' => $this->survey->id], 'id');
        foreach ($fbsections as $fbsid => $fbsection) {
            ...
            unset($fbsection->id);
            $newfbsid = $DB->insert_record('observation_fb_sections', $fbsection);
            $feedbackrecs = $DB->get_records('observation_feedback', ['sectionid' => $fbsid], 'id');
            foreach ($feedbackrecs as $feedbackrec) {
                $feedbackrec->sectionid = $newfbsid;
                unset($feedbackrec->id);
                $DB->insert_record('observation_feedback', $feedbackrec);
            }
        }
    ...
    private function response_check_format($section, $formdata, $checkmissing = true, $checkwrongformat = true) {
        ...
        foreach ($this->questionsbysec[$section] as $questionid) {
            ...
            if (!$this->questions[$questionid]->response_complete($formdata)) {
                $missing++;
                $strmissing .= get_string('num', 'observation').$qnum.'. ';
            }
            if (!$this->questions[$questionid]->response_valid($formdata)) {
                $wrongformat++;
                $strwrongformat .= get_string('num', 'observation').$qnum.'. ';
            }
        }
        $message = '';
        $nonumbering = false;
        ...
        if ($checkmissing && $missing) {
            ...
            if ($missing == 1) {
                $message = get_string('missingquestion', 'observation').$strmissing;
            } else {
                $message = get_string('missingquestions', 'observation').$strmissing;
            }
            if ($wrongformat) {
                $message .= '<br />';
            }
        ...
        if ($checkwrongformat && $wrongformat) {
            if ($nonumbering) {
                $message .= get_string('wronganswers', 'observation');
            } else {
                if ($wrongformat == 1) {
                    $message .= get_string('wrongformat', 'observation').$strwrongformat;
                } else {
                    $message .= get_string('wrongformats', 'observation').$strwrongformat;
                }
            }
        }
        return ($message);
    ...
    private function response_delete($rid, $sec = null) {
        ...
        foreach (array('response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_text',
            ...
            $DB->delete_records_select('observation_'.$tbl, $select, $params);
        }
    }
    ...
    private function response_commit($rid) {
        ...
        return $DB->update_record('observation_response', $record);
    }
    ...
    public function get_latest_responseid($userid) {
        ...
        $params = ['observationid' => $this->id, 'userid' => $userid, 'complete' => 'n'];
        if ($records = $DB->get_records('observation_response', $params, 'submitted DESC', 'id,observationid', 0, 1)) {
            $rec = reset($records);
            return $rec->id;
        } else {
            return 0;
    ...
    private function response_select_max_sec($rid) {
        ...
        $max = $DB->count_records_select('observation_question', $select, $params) + 1;
    ...
    private function response_select_max_pos($rid) {
        ...
        foreach (array('response_bool', 'resp_single', 'resp_multiple', 'response_rank', 'response_text',
                     'response_other', 'response_date') as $tbl) {
            $sql = 'SELECT MAX(q.position) as num FROM {observation_'.$tbl.'} a, {observation_question} q '.
     ...
     *
     */
    private function submission_notify($rid, $email='', $studentid='') {
        global $DB, $USER;
        ...
        if (empty($email)) {
            ...
            if (isset($this->survey)) {
                if (isset($this->survey->email)) {
                    ...
                    $email = $DB->get_field('observation_survey', 'email', ['id' => $this->survey->id]);
                }
            } else {
                $email = '';
            }
        ...
        /*if (!empty($email)) {
            // Response to student
            $success = $this->response_send_email($rid, $email, $studentid);
        }*/

        if (!empty($this->notifications)) {
            // Handle notification of submissions.
            $success = $this->send_submission_notifications($rid, $studentid) && $success;
        }
     ...
     *
     */
    private function send_submission_notifications($rid, $studentid = '') {
        global $CFG, $USER;
        ...
        $success = true;
        $user = $USER;//($studentid) ? \core_user::get_user($studentid) : $USER;
        ...
        if ($notifyusers = $this->get_notifiable_users($USER->id)) {
            ...
            $info->submissionurl = $CFG->wwwroot.'/mod/observation/report.php?action=vresp&amp;sid='.$this->survey->id.'&amp;rid='.$rid.'&amp;instance='.$this->id;

            $info->coursename = $this->course->fullname;

            $info->postsubject = get_string('submissionnotificationsubject', 'observation');
            $info->posttext = get_string($langstringtext, 'observation', $info);
            $info->posthtml = '<p>' . get_string($langstringhtml, 'observation', $info) . '</p>';
            if (!empty($message)) {
                $info->posttext .= html_to_text($message);
                $info->posthtml .= $message;
            }

// print_object($notifyusers);
            foreach ($notifyusers as $notifyuser) {
                $info->userto = $notifyuser;
                if ($studentid == $notifyuser->id) {
                    $info->submissionurl = $CFG->wwwroot.'/mod/observation/myreport.php?action=vresp&user='.$notifyuser->id.'&instance='.$this->id;
                    $info->posttext = get_string($langstringtext, 'observation', $info);
                    $info->posthtml = '<p>' . get_string($langstringhtml, 'observation', $info) . '</p>';
                    if (!empty($message)) {
                        $info->posttext .= html_to_text($message);
    ...
    private function send_message($info, $eventtype) {
        ...
        $eventdata->modulename       = 'observation';
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, 'mod/observation:submissionnotification',
    ...
    private function get_full_submission_for_notifications($rid) {
        ...
        foreach ($responses as $response) {
            ...
            $message .= get_string('answers', 'observation') . ":<br />\n";
     ...
     * @throws dml_exception
     */
    private function response_send_email($rid, $email, $isstudent=false) {
        global $CFG;
        ...
        $subject = get_string('surveyresponse', 'observation') .": $name [$rid]";
        if ($isstudent) {
            $url = $CFG->wwwroot.'/mod/observation/myreport.php?action=vresp&amp;byresponse=1&amp;user='.$isstudent.'&amp;instance='.$this->id;
        } else {
            $url = $CFG->wwwroot.'/mod/observation/report.php?action=vresp&amp;sid='.$this->survey->id.'&amp;rid='.$rid.'&amp;instance='.$this->id;
        }

        // Html and plaintext body.
        $bodyhtml       = get_string('surveyresponse', 'observation') .' "'.$name.'"'.$endhtml;
        $bodyplaintext  = get_string('surveyresponse', 'observation') .' "'.$name.'"'.$endplaintext;
        ...
        if ($isstudent) {            
            $user = \core_user::get_user($isstudent);
            $bodyhtml .= get_string('observation_student', 'mod_observation').': '.fullname($user). $endhtml;
            $bodyhtml .= get_string('respondent', 'mod_observation').': '.fullname($USER). $endhtml;
            $bodyplaintext .= get_string('email').':'. $user->email. $endplaintext;
            $bodyplaintext .= get_string('email').':'. $USER->email. $endplaintext;            
        }
        ...
        // echo $bodyhtml;
/*        print_object($mailaddresses);

        exit;*/
        foreach ($mailaddresses as $email) {

            $userto = \core_user::get_user_by_email($email);
            //new stdClass();
    ...
    public function response_insert($responsedata, $userid, $resume=false) {
        ...
        if (empty($responsedata->rid)) {
            ...
            $record->observationid = $this->id;
            $record->userid = $userid;
            $record->trainer_id = $USER->id;
            $responsedata->rid = $DB->insert_record('observation_response', $record);
        ...
        if ($resume) {
            ...
            $event = \mod_observation\event\attempt_saved::create($params);
            $event->trigger();
        }
    ...
    private function response_select($rid) {
        ...
        $values = \mod_observation\responsetype\boolean::response_select($rid);

        // Response_single (radio button or dropdown).
        $values += \mod_observation\responsetype\single::response_select($rid);

        // Response_multiple.
        $values += \mod_observation\responsetype\multiple::response_select($rid);

        // Response_rank.
        $values += \mod_observation\responsetype\rank::response_select($rid);

        // Response_text.
        $values += \mod_observation\responsetype\text::response_select($rid);

        // Response_date.
        $values += \mod_observation\responsetype\date::response_select($rid);
    ...
    private function response_goto_thankyou() {
        ...
        if ($result = $DB->get_record_select('observation_survey', $select, null, $fields)) {
        ...
        if (empty($thankhead)) {
            $thankhead = get_string('thank_head', 'observation');
        }
        ...
        $this->page->add_to_page('addinfo',
            format_text(file_rewrite_pluginfile_urls($thankbody, 'pluginfile.php',
                $this->context->id, 'mod_observation', 'thankbody', $this->survey->id), FORMAT_HTML, ['noclean' => true]));
    ...
    private function response_goto_saved($url) {
        ...
        $savedprogress = get_string('savedprogress', 'observation', '<strong>'.$resumesurvey.'</strong>');
    ...
    public function survey_results_navbar_alpha($currrid, $currentgroupid, $cm, $byresponse) {
        ...
        // Is this observation set to fullname or anonymous?
        ...
        $url = $CFG->wwwroot.'/mod/observation/report.php?action=vresp&group='.$currentgroupid.'&individualresponse=1';
        if (!$byresponse) {     // Display navbar.
            ...
            $url = $CFG->wwwroot.'/mod/observation/report.php?action=vresp&byresponse=1&group='.$currentgroupid;
            // Display navbar.
            $navbar->listlink = $url;

            // Display a "print this response" icon here in prevision of total removal of tabs in version 2.6.
            $linkname = '&nbsp;'.get_string('print', 'observation');
            $url = '/mod/observation/print.php?qid='.$this->id.'&rid='.$currrid.
                '&courseid='.$this->course->id.'&sec=1';
            $title = get_string('printtooltip', 'observation');
            ...
            for ($i = 0; $i < $total; $i++) {
                if ($isfullname) {
                    ...
                    $resparr[] = '<a title = "'.$responsedate.'" href="'.$url.'&amp;rid='.
                        ...
                        get_string('response', 'observation').($i + 1).'</a> ';
                }
            }
    ...
    public function survey_results($precision = 1, $showtotals = 1, $qid = '', $cids = '', $rid = '',
        ...
        $SESSION->observation->noresponses = false;
        if (empty($precision)) {
            $precision  = 1;
        }
        if ($showtotals === '') {
        ...
        $haschoices = array();
        $responsetable = array();
        if (!($types = $DB->get_records('observation_question_type', array(), 'typeid', 'typeid, has_choices, response_table'))) {
            $errmsg = sprintf('%s [ %s: question_type ]',
                get_string('errortable', 'observation'), 'Table');
            return($errmsg);
        }
        ...
        if (empty($this->survey)) {
            $errmsg = get_string('erroropening', 'observation') ." [ ID:{$this->sid} R:";
            return($errmsg);
        }

        if (empty($this->questions)) {
            $errmsg = get_string('erroropening', 'observation') .' '. 'No questions found.';
            return($errmsg);
        }
        ...
        if (!empty($rid)) {
            ...
            if (!$rows) {
                $this->page->add_to_page('respondentinfo',
                    $this->renderer->notification(get_string('noresponses', 'observation'),
                        \core\output\notification::NOTIFY_ERROR));
                $SESSION->observation->noresponses = true;
                return;
            }
            $numresps = count($rows);
            $this->page->add_to_page('respondentinfo',
                ' '.get_string('responses', 'observation').': <strong>'.$numresps.'</strong>');
            if (empty($rows)) {
                $errmsg = get_string('erroropening', 'observation') .' '. get_string('noresponsedata', 'observation');
                return($errmsg);
            }
        ...
        foreach ($this->questions as $question) {
            ...
            $this->page->add_to_page('responses',
                $this->renderer->container(format_text(file_rewrite_pluginfile_urls($question->content, 'pluginfile.php',
                    $question->context->id, 'mod_observation', 'question', $question->id),
    ...
    /**
     * Return all the fields to be used for users in observation sql.
     *
     * @author: Guy Thomas
     * @return array|string
     */
    ...
    protected function get_survey_all_responses($rid = '', $userid = '', $groupid = false, $showincompletes = 0) {
        ...
        // If a observation is "public", and this is the master course, need to get responses from all instances.
        if ($this->survey_is_public_master()) {
            $qids = array_keys($DB->get_records('observation', ['sid' => $this->sid], 'id'));
        } else {
            $qids = $this->id;
        }

        foreach ($uniquetypes as $type) {
            $question = \mod_observation\question\question::question_builder($type);
    ...
    protected function process_csv_row(array &$row,
        ...
        if ($config === null) {
            $config = get_config('observation', 'downloadoptions');
        }
        ...
        // Moodle:
        // Get the course name that this observation belongs to.
        if (!$this->survey_is_public()) {
            $courseid = $this->course->id;
        ...
        if ($isanonymous) {
            ...
            $fullname = get_string('anonymous', 'observation') . $anonumap[$user->id];
            $username = '';
            $uid = '';
        } else {
            $uid = $user->id;
        ...
        if (in_array('submitted', $options)) {
            ...
            $submitted = date(get_string('strfdateformatcsv', 'observation'), $resprow->submitted);
    ...
    */
    public function generate_csv($rid='', $userid='', $choicecodes=1, $choicetext=0, $currentgroupid, $showincompletes = 0) {
        ...
        $output = array();
        $stringother = get_string('other', 'observation');
        ...
        foreach ($options as $option) {
            if (in_array($option, array('response', 'submitted', 'id'))) {
                $columns[] = get_string($option, 'observation');
                $types[] = 0;
            } else {
        ...
        if (!$survey = $DB->get_record('observation_survey', array('id' => $this->survey->id))) {
        ...
        if (count(array_intersect($choicetypes, $uniquetypes)) > 0) {
            ...
            $choicesql = "
                SELECT DISTINCT c.id as cid, q.id as qid, q.precise AS precise, q.name, c.content
                  FROM {observation_question} q
                  JOIN {observation_quest_choice} c ON question_id = q.id
                ...
                switch ($type) {
                    ...
                    case QUESDROP:
                        ...
                        foreach ($choices as $choice) {
                            ...
                            if (\mod_observation\question\choice\choice::content_is_other_choice($content)) {
                    ...
                    case QUESCHECK: // Multiple.
                        ...
                        foreach ($choices as $choice) {
                            $content = $choice->content;
                            $modality = '';
                            $contents = observation_choice_values($content);
                            if ($contents->modname) {
                    ...
                    case QUESRATE: // Rate.
                        foreach ($choices as $choice) {
                            ...
                            $content = $choice->content;
                            $osgood = false;
                            if (\mod_observation\question\rate::type_is_osgood_rate_scale($choice->precise)) {
                                $osgood = true;
                            }
                            if (preg_match("/^[0-9]{1,3}=/", $content, $ndd)) {
                                ...
                                if ($osgood) {
                                    ...
                                    $contents = observation_choice_values($contentleft);
                                    if ($contents->title) {
                                        $contentleft = $contents->title;
                                    }
                                    $contents = observation_choice_values($contentright);
                                    if ($contents->title) {
            ...
            if ($qtype === QUESRATE || $qtype === QUESCHECK) {
                ...
                if ($questionobj->has_choices()) {
                    ...
                    if (\mod_observation\question\choice\choice::content_is_other_choice($content)) {
                        ...
                        $responsetxt = \mod_observation\question\choice\choice::content_other_choice_display($content);
        ...
        if (is_array($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                if ($question->id == $movequestion->id) {
                    ...
                    $DB->update_record("observation_question", $movequestion);
                    continue;
                }
                $question->position = $index;
                $DB->update_record("observation_question", $question);
                $index++;
            }
            return true;
        }
    ...
    public function response_analysis($rid, $resps, $compare, $isgroupmember, $allresponses, $currentgroupid,
        ...
        require_once($CFG->dirroot.'/mod/observation/drawchart.php');

        // Find if there are any feedbacks in this observation.
        $sql = "SELECT * FROM {observation_fb_sections} WHERE surveyid = ? AND section IS NOT NULL";
        ...
        $resp = $DB->get_record('observation_response', ['id' => $rid]);
        if (!empty($resp)) {
            $userid = $resp->userid;
        ...
        if ($this->survey->feedbackscores) {
            ...
            $table->wrap = [];
            if ($compare) {
                $table->head = [get_string('feedbacksection', 'observation'), $ruser, $groupname];
            } else {
                $table->head = [get_string('feedbacksection', 'observation'), $groupname];
            }
        }
        ...
        // Calculate max score per question in observation.
        $qmax = [];
        $maxtotalscore = 0;
        foreach ($this->questions as $question) {
            $qid = $question->id;
        ...
        if ($this->survey->feedbacksections == 1) {
            ...
            $feedbacks = $DB->get_records('observation_feedback', ['sectionid' => $sectionid]);
            $labels = array();
            ...
            $feedback = $DB->get_record_select('observation_feedback',
            ...
            $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                $this->context->id, 'mod_observation', 'sectionheading', $sectionid);
            ...
            if (!empty($feedback->feedbacktext)) {
                ...
                $feedbacktext = file_rewrite_pluginfile_urls($feedback->feedbacktext, 'pluginfile.php',
                    $this->context->id, 'mod_observation', 'feedback', $feedback->id);
        ...
        for ($section = 1; $section <= $numsections; $section++) {
            ...
            if (!$allresponses) {
                ...
                $formatoptions->noclean = true;
                $sectionheading = file_rewrite_pluginfile_urls($sectionheading, 'pluginfile.php',
                    $this->context->id, 'mod_observation', 'sectionheading', $imageid);
    ...
    public function save_mobile_data($userid, $sec, $completed, $rid, $submit, $action, array $responses) {
        ...
        if ($action == 'nextpage') {
            ...
            // If reviewing a completed observation, don't insert a response.
            $msg = $this->response_check_format($response->sec, $response);
            if (empty($msg)) {
