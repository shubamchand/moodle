...
$instance = optional_param('instance', false, PARAM_INT);   // Observation ID.
...
if ($instance === false) {
    if (!empty($SESSION->instance)) {
        ...
        print_error('requiredparameter', 'observation');
    }
}
...
$SESSION->observation->current_tab = 'allreport';
...
$observationgroups = '';
$groupscount = 0;
$SESSION->observation->respscount = 0;
$SESSION->observation_surveyid = $sid;
...
switch ($action) {
    ...
    case 'dresp':  // Delete individual response? Ask for confirmation.
        ...
        require_capability('mod/observation:deleteresponses', $context);

        if (empty($observation->survey)) {
            $id = $observation->survey;
            notify ("observation->survey = /$id/");
            print_error('surveynotexists', 'observation');
        } else if ($observation->survey->courseid != $course->id) {
            print_error('surveyowner', 'observation');
        } else if (!$rid || !is_numeric($rid)) {
            print_error('invalidresponse', 'observation');
        } else if (!($resp = $DB->get_record('observation_response', array('id' => $rid)))) {
            print_error('invalidresponserecord', 'observation');
        }
        ...
        if (!empty($resp->userid)) {
            if ($user = $DB->get_record('user', ['id' => $resp->userid])) {
                ...
                $ruser = '- '.get_string('unknown', 'observation').' -';
            }
        } else {
            $ruser = $resp->userid;
        }
        ...
        $PAGE->set_title(get_string('deletingresp', 'observation'));
        $PAGE->set_heading(format_string($course->fullname));
        echo $observation->renderer->header();

        // Print the tabs.
        $SESSION->observation->current_tab = 'deleteresp';
        include('tabs.php');

        $timesubmitted = '<br />'.get_string('submitted', 'observation').'&nbsp;'.userdate($resp->submitted);
        ...
        $msg .= get_string('confirmdelresp', 'observation', $ruser.$timesubmitted);
        $msg .= '</div>';
        ...
        $observation->page->add_to_page('notifications', $observation->renderer->confirm($msg, $buttonyes, $buttonno));
    ...
    case 'delallresp': // Delete all responses? Ask for confirmation.
        ...
        if (!empty($respsallparticipants)) {
            ...
            // Print the tabs.
            $SESSION->observation->current_tab = 'deleteall';
            include('tabs.php');

            // Print the confirmation.
            $msg = '<div class="warning centerpara">';
            if ($groupmode == 0) {   // No groups or visible groups.
                $msg .= get_string('confirmdelallresp', 'observation');
            } else {                 // Separate groups.
                $msg .= get_string('confirmdelgroupresp', 'observation', $groupname);
            }
            $msg .= '</div>';
    ...
    case 'dvresp': // Delete single response. Do it!
        ...
        if (empty($observation->survey)) {
            ...
            print_error('invalidresponse', 'observation');
        } else if (!($response = $DB->get_record('observation_response', array('id' => $rid)))) {
            print_error('invalidresponserecord', 'observation');
        }

        if (observation_delete_response($response, $observation)) {
            if (!$DB->count_records('observation_response', array('observationid' => $observation->id, 'complete' => 'y'))) {
                $redirection = $CFG->wwwroot.'/mod/observation/view.php?id='.$cm->id;
            } else {
                $redirection = $CFG->wwwroot.'/mod/observation/report.php?action=vresp&amp;instance='.
                    $instance.'&amp;byresponse=1';
            }

            // Log this observation delete single response action.
            $params = array('objectid' => $observation->survey->id,
                'context' => $observation->context,
                'courseid' => $observation->course->id,
                'relateduserid' => $response->userid);
            $event = \mod_observation\event\response_deleted::create($params);
            $event->trigger();
            ...
            error (get_string('couldnotdelresp', 'observation').$rid.get_string('by', 'observation').$ruser.'?',
                $CFG->wwwroot.'/mod/observation/report.php?action=vresp&amp;sid='.$sid.'&amp;&amp;instance='.
                $instance.'byresponse=1');
        }
        break;

    case 'dvallresp': // Delete all responses in observation (or group). Do it!
        ...
        if ($groupmode > 0) {
            switch ($currentgroupid) {
                ...
                default:     // Members of a specific group.
                    if (!($resps = $observation->get_responses(false, $currentgroupid))) {
                        $resps = [];
                    }
            }
            if (empty($resps)) {
                ...
                if ($rid === false) {
                    ...
                    $resp = $DB->get_record('observation_response', array('id' => $rid));
                }
                if (!empty($resp->userid)) {
                    if ($user = $DB->get_record('user', ['id' => $resp->userid])) {
                        $ruser = fullname($user);
        ...
        if (!empty($resps)) {
            ...
            if (!$observation->count_submissions()) {
                $redirection = $CFG->wwwroot.'/mod/observation/view.php?id='.$cm->id;
            } else {
                $redirection = $CFG->wwwroot.'/mod/observation/report.php?action=vall&amp;sid='.$sid.'&amp;instance='.$instance;
            }

            // Log this observation delete all responses action.
            $context = context_module::instance($observation->cm->id);
            $anonymous = $observation->respondenttype == 'anonymous';

            $event = \mod_observation\event\all_responses_deleted::create(array(
            ...
            error (get_string('couldnotdelresp', 'observation'),
                $CFG->wwwroot.'/mod/observation/report.php?action=vall&amp;sid='.$sid.'&amp;instance='.$instance);
        }
        break;

    case 'dwnpg': // Download page options.

        require_capability('mod/observation:downloadresponses', $context);
        ...
        // Tab setup.
        if (empty($user)) {
            $SESSION->observation->current_tab = 'downloadcsv';
        } else {
            $SESSION->observation->current_tab = 'mydownloadcsv';
        }
            ...
            switch ($currentgroupid) {
                ...
                default:     // Members of a specific group.
                    $groupname = get_string('membersofselectedgroup', 'group').' '.get_string('group').' '.
                        $observationgroups[$currentgroupid]->name;
            }
        }
        $output = '';
        $output .= "<br /><br />\n";
        $output .= $observation->renderer->help_icon('downloadtextformat', 'observation');
        $output .= '&nbsp;' . (get_string('downloadtextformat', 'observation')) . ':&nbsp;' .
            get_string('responses', 'observation').'&nbsp;'.$groupname;
        $output .= $observation->renderer->heading(get_string('textdownloadoptions', 'observation'));
        $output .= $observation->renderer->box_start();
        $output .= "<form action=\"{$CFG->wwwroot}/mod/observation/report.php\" method=\"GET\">\n";
        ...
        $output .= html_writer::checkbox('choicecodes', 1, true, get_string('includechoicecodes', 'observation'));
        $output .= "<br />\n";
        $output .= html_writer::checkbox('choicetext', 1, true, get_string('includechoicetext', 'observation'));
        $output .= "<br />\n";
        $output .= html_writer::checkbox('complete', 1, false, get_string('includeincomplete', 'observation'));
        $output .= "<br />\n";
        $output .= "<br />\n";
        $output .= "<input type=\"submit\" name=\"submit\" value=\"".get_string('download', 'observation')."\" />\n";
        $output .= "</form>\n";
        $output .= $observation->renderer->box_end();

        $observation->page->add_to_page('respondentinfo', $output);
        echo $observation->renderer->render($observation->page);

        echo $observation->renderer->footer('none');

        // Log saved as text action.
        $params = array('objectid' => $observation->id,
            'context' => $observation->context,
            'courseid' => $course->id,
            'other' => array('action' => $action, 'instance' => $instance, 'currentgroupid' => $currentgroupid)
        );
        $event = \mod_observation\event\all_responses_saved_as_text::create($params);
        $event->trigger();
    ...
    case 'dcsv': // Download responses data as text (cvs) format.
        ...
        // Use the observation name as the file name. Clean it and change any non-filename characters to '_'.
        $name = clean_param($observation->name, PARAM_FILE);
        ...
        $output = $observation->generate_csv('', $user, $choicecodes, $choicetext, $currentgroupid, $showincompletes);
    ...
    case 'vallarsort':   // View all responses sorted in descending order.
        ...
        $PAGE->set_heading(format_string($course->fullname));
        echo $observation->renderer->header();
        if (!$observation->capabilities->readallresponses && !$observation->capabilities->readallresponseanytime) {
            // Should never happen, unless called directly by a snoop.
            print_error('nopermissions', '', '', get_string('viewallresponses', 'observation'));
            // Finish the page.
            echo $observation->renderer->footer($course);
            break;
        }
        ...
        switch ($action) {
            case 'vallasort':
                $SESSION->observation->current_tab = 'vallasort';
                break;
            case 'vallarsort':
                $SESSION->observation->current_tab = 'vallarsort';
                break;
            default:
                $SESSION->observation->current_tab = 'valldefault';
        }
        include('tabs.php');

        $respinfo = '';
        $resps = array();
        // Enable choose_group if there are observation groups and groupmode is not set to "no groups"
        // and if there are more goups than 1 (or if user can view all groups).
        if (is_array($observationgroups) && $groupmode > 0) {
            $groupselect = groups_print_activity_menu($cm, $url->out(), true);
            // Count number of responses in each group.
            foreach ($observationgroups as $group) {
                $respscount = $observation->count_submissions(false, $group->id);
        ...
        if (!empty($resps)) {
            ...
            $feedbackmessages = $observation->response_analysis(0, $resps, false, false, true, $currentgroupid);
            ...
            if ($feedbackmessages) {
                ...
                $observation->page->add_to_page('feedbackmessages', $msgout);
            }
        }
        ...
        $event = \mod_observation\event\all_responses_viewed::create($params);
        $event->trigger();

        $respinfo .= get_string('viewallresponses', 'observation').'. '.$groupname.'. ';
        $strsort = get_string('order_'.$sort, 'observation');
        $respinfo .= $strsort;
        $respinfo .= $observation->renderer->help_icon('orderresponses', 'observation');
        $observation->page->add_to_page('respondentinfo', $respinfo);

        $ret = $observation->survey_results(1, 1, '', '', '', false, $currentgroupid, $sort);
    ...
    default:
            ...
            if ($groupmode > 0) {
                switch ($currentgroupid) {
                    case 0:     // All participants.
                        ...
                        break;
                    default:     // Members of a specific group.
                        $resps = $observation->get_responses(false, $currentgroupid);
                }
                if (empty($resps)) {
                    $noresponses = true;
                } else {
        ...
        // Print the tabs.
        if ($byresponse) {
            $SESSION->observation->current_tab = 'vrespsummary';
        }
        if ($individualresponse) {
            $SESSION->observation->current_tab = 'individualresp';
        }
        include('tabs.php');
        ...
        if ($noresponses) {
            $observation->page->add_to_page('respondentinfo',
                get_string('group').' <strong>'.groups_get_group_name($currentgroupid).'</strong>: '.
                get_string('noresponses', 'observation'));
        } else {
            $groupname = get_string('group').': <strong>'.groups_get_group_name($currentgroupid).'</strong>';
            if ($currentgroupid == 0 ) {
                $groupname = get_string('allparticipants');
            ...
            if ($byresponse) {
                $respinfo = '';
                $respinfo .= $observation->renderer->box_start();
                $respinfo .= $observation->renderer->help_icon('viewindividualresponse', 'observation').'&nbsp;';
                $respinfo .= get_string('viewindividualresponse', 'observation').' <strong> : '.$groupname.'</strong>';
                $respinfo .= $observation->renderer->box_end();
                $observation->page->add_to_page('respondentinfo', $respinfo);
            }
            $observation->survey_results_navbar_alpha($rid, $currentgroupid, $cm, $byresponse);
            if (!$byresponse) { // Show respondents individual responses.
                $observation->view_response($rid, '', false, $resps, true, true, false, $currentgroupid);
            }
        }
